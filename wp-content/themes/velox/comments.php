	...
	if ( have_comments() ) :
		...
		<h2 class="comments-title">
			...
			if ( '1' === $velox_comment_count ) {
				printf(
					/* translators: 1: title. */
					esc_html__( 'Один коментар &ldquo;%1$s&rdquo;', 'velox' ),
					'<span>' . wp_kses_post( get_the_title() ) . '</span>'
				);
			} else {
				printf(
					/* translators: 1: comment count number, 2: title. */
					esc_html( _nx( 'Декілька(%1$s) коментарів &ldquo;%2$s&rdquo;', 'Декілька(%1$s) коментарів &ldquo;%2$s&rdquo;', $velox_comment_count, 'comments title', 'velox' ) ),
