...
if ( ! function_exists( 'velox_posted_on' ) ) :
	...
	function velox_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Дата публікації %s', 'post date', 'velox' ),
...
if ( ! function_exists( 'velox_posted_by' ) ) :
	...
	function velox_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'створив запис користувач: %s', 'post author', 'velox' ),
	...
	function velox_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Написано в: %1$s', 'velox' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'velox' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
